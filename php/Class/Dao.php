...
class Dao {
    ...
    public static function ajouterPersonne($nom, $prenom, $adr, $tele, $email, $image, $nom_de_class) {
        ...
        $sql = "INSERT INTO `{$nomTable}`(nom,prenom,adr,tele,email, image) VALUES (?,?,?,?,?,?)";
    ...
    public static function modifierPersonne($id, $nom, $prenom, $adr, $tele, $email, $image, $nom_de_class) {
        $pdo = Dao::getPDO();
        $nomTable = strtolower($nom_de_class);
        $sql = "UPDATE `{$nomTable}` SET nom=?, prenom=?, adr=?, tele=?, email=?, image=? WHERE id=?";
    ...
    static public function nbrDesTuples($nom_de_class) {
        $pdo = Dao::getPDO();
        $nomTable = strtolower($nom_de_class);
        $sql = "SELECT count(id) as nbrDesTuples FROM `{$nomTable}`;";
    ...
    public static function modifierAdmin($id, $nom, $prenom, $adr, $tele, $email, $mdp, $nom_de_class) {
        $pdo = Dao::getPDO();
        $nomTable = strtolower($nom_de_class);
        $sql = "UPDATE `{$nomTable}` SET nom=?, prenom=?, adr=?, tele=?, email=?, mdp=? WHERE id=?";
    ...
    public static function displayAllPur() {
        $pdo = Dao::getPDO();
        // Agréger par numéro d'approvisionnement ; supprimer num_pr (plusieurs produits par approvisionnement)
        // et inclure les colonnes non-agrégées dans le GROUP BY pour respecter ONLY_FULL_GROUP_BY
        $sql = "SELECT SUM(qte_achete * prix_achat) AS total, num_app, date_app, nom, prenom, image
        FROM (((est_compose NATURAL JOIN produit) NATURAL JOIN approvisionnement) NATURAL JOIN fournisseur)
        WHERE id = id_four
        GROUP BY num_app, date_app, nom, prenom, image
        ORDER BY num_app;";
        return $pdo->query($sql)->fetchAll();
    }
    ...
    public static function displayAllSales() {
        ...
        $pdo = Dao::getPDO();
    // Aggregate by sale (num_com). Remove non-aggregated num_pr and include
    // non-aggregated columns in the GROUP BY to satisfy ONLY_FULL_GROUP_BY.
    $sql = "SELECT SUM(qte_pr * prix_vente) AS total, num_com, date_com, nom, prenom, image
FROM (((contient_pr NATURAL JOIN produit) NATURAL JOIN commande) NATURAL JOIN client)
WHERE id_cli = id
GROUP BY num_com, date_com, nom, prenom, image
ORDER BY total DESC;";
    return $pdo->query($sql)->fetchAll();
    }
    ...
    public static function topSales() {
        $pdo = Dao::getPDO();
    // Regrouper par commande et inclure les colonnes non-agrégées dans le GROUP BY
    // Retirer `num_pr` (plusieurs produits par commande) et éviter les colonnes dupliquées
    $sql = "SELECT SUM(qte_pr * prix_vente) AS total, num_com, date_com, nom, prenom, image
FROM (((contient_pr NATURAL JOIN produit) NATURAL JOIN commande) NATURAL JOIN client)
WHERE id_cli = id
GROUP BY num_com, date_com, nom, prenom, image
ORDER BY total DESC;";
    ...
    // ==================== CLIENT AUTH & ECOMMERCE METHODS ====================

    // Get client by email for authentication
    public static function clientByEmail($email) {
        $pdo = Dao::getPDO();
    ...
    public static function updateClientProfile($id, $nom, $prenom, $adr, $tele, $email, $image) {
        $pdo = Dao::getPDO();
        $sql = "UPDATE client SET nom=?, prenom=?, adr=?, tele=?, email=?, image=? WHERE id=?";
        $pdo->prepare($sql)->execute([$nom, $prenom, $adr, $tele, $email, $image, $id]);
    }
    ...
    // Get client orders
    public static function getClientOrders($id_client) {
        $pdo = Dao::getPDO();
        $sql = "SELECT c.*, SUM(cp.qte_pr * cp.prix_vente) as total 
                FROM commande c 
    ...
    // Get order details for client
    public static function getOrderDetails($num_com, $id_client) {
        $pdo = Dao::getPDO();
        $sql = "SELECT p.*, cp.qte_pr, cp.prix_vente, (cp.qte_pr * cp.prix_vente) as subtotal
                FROM contient_pr cp
                JOIN produit p ON cp.num_pr = p.num_pr
                JOIN commande c ON cp.num_com = c.num_com
                WHERE c.num_com = ? AND c.id_cli = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$num_com, $id_client]);
        return $stmt->fetchAll();
    }

    // Create order with payment and delivery info
    public static function createClientOrder($id_client, $adr_livraison, $mode_paiement, $total_amount) {
        $pdo = Dao::getPDO();
    ...
    // Add product to cart (session-based, but can also persist to DB)
    public static function addToDbCart($id_client, $num_pr, $qte = 1) {
        $pdo = Dao::getPDO();
        // Check if already in cart
        $sql = "SELECT * FROM panier WHERE id_client = ? AND num_pr = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_client, $num_pr]);
        $existing = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($existing) {
            // Update quantity
            $sql = "UPDATE panier SET qte = qte + ? WHERE id_client = ? AND num_pr = ?";
            $pdo->prepare($sql)->execute([$qte, $id_client, $num_pr]);
        } else {
            // Insert new
            $sql = "INSERT INTO panier(id_client, num_pr, qte) VALUES (?,?,?)";
            $pdo->prepare($sql)->execute([$id_client, $num_pr, $qte]);
        }
    }

    // Get cart items for client
    public static function getCartItems($id_client) {
        $pdo = Dao::getPDO();
        $sql = "SELECT p.*, pa.qte, pa.id_panier, (p.prix_uni * pa.qte) as subtotal
                FROM panier pa
                JOIN produit p ON pa.num_pr = p.num_pr
                WHERE pa.id_client = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_client]);
        return $stmt->fetchAll();
    ...
    public static function clearCart($id_client) {
        $pdo = Dao::getPDO();
        $sql = "DELETE FROM panier WHERE id_client = ?";
        $pdo->prepare($sql)->execute([$id_client]);
    }

    // Remove item from cart
    public static function removeFromCart($id_panier, $id_client) {
        $pdo = Dao::getPDO();
        $sql = "DELETE FROM panier WHERE id_panier = ? AND id_client = ?";
        $pdo->prepare($sql)->execute([$id_panier, $id_client]);
    }

    // Update cart item quantity
    public static function updateCartQuantity($id_panier, $id_client, $qte) {
        $pdo = Dao::getPDO();
        if ($qte <= 0) {
            Dao::removeFromCart($id_panier, $id_client);
        } else {
            $sql = "UPDATE panier SET qte = ? WHERE id_panier = ? AND id_client = ?";
            $pdo->prepare($sql)->execute([$qte, $id_panier, $id_client]);
        }
    }
}